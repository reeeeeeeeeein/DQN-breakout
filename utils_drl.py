...
class Agent(object):
    ...
    def get_pri_val(self,nowstate,action,reward,done,nextstate):
       # print(nowstate.shape,action.shape,reward.shape,nextstate.shape)
        with torch.no_grad():
            values = self.__policy(nowstate.float()).gather(1, action)
